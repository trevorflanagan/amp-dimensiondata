...
public abstract class FirewallRule {
    ...
    FirewallRule() {} // For AutoValue only!
    ...
    public static FirewallRule create(String id, String name, String ruleType, String networkDomainId, String state, String action, String ipVersion, String protocol, String datacenterId, FirewallRuleTarget source, FirewallRuleTarget destination, Boolean enabled, Placement placement) {
        return builder().id(id).name(name).ruleType(ruleType).networkDomainId(networkDomainId).state(state).action(action).ipVersion(ipVersion).protocol(protocol).datacenterId(datacenterId).source(source).destination(destination).enabled(enabled).placement(placement).build();
    }
